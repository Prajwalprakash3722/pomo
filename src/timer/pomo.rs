...
pub struct PomodoroTimer {
    config: PomoConfig,
    duration_left_in_state: Duration,
...
impl Timer for PomodoroTimer {
    fn tick(&mut self, time_elapsed_since: Duration) -> TimerResult<()> {
        let sub_duration = self.duration_left_in_state.checked_sub(time_elapsed_since);
        match sub_duration {
            None => {
                let carry_over = time_elapsed_since - self.duration_left_in_state;
            ...
            Some(sub_duration) => {
                self.duration_left_in_state = sub_duration
            }
        }
        Ok(())
    }
    ...
    fn get_duration_left(&self) -> Duration { self.duration_left_in_state }

    fn set_duration_left(&mut self, duration_left: Duration) -> TimerResult<()> {
        self.duration_left_in_state = duration_left;
        Ok(())
    }
...
impl PomodoroTimer {
    ...
    fn change_state_and_reset_time(&mut self) {
        ...
        self.duration_left_in_state = self.mode.get_new_time_left_millis_for_state(&self.config)
    }
...
mod test {
    ...
    #[test]
    pub fn regular_tick() {
        let ppomoconfig = PomoConfig::default();
        let mut timer_state = PomodoroTimer {
            duration_left_in_state: Duration::from_millis(1000),
            mode: Break,
            cycles_done: 0,
        ...
        assert_eq!(Duration::from_millis(750), timer_state.duration_left_in_state);
    ...
    pub fn sucessive_ticks() {
        let ppomoconfig = PomoConfig::default();
        let mut timer_state = PomodoroTimer {
            duration_left_in_state: Duration::from_millis(1004),
            mode: Break,
            cycles_done: 0,
            config: ppomoconfig.clone(),
        };
        ...
        assert_eq!(Break, timer_state.mode);
        assert_eq!(Duration::from_millis(4), timer_state.duration_left_in_state);
        assert_eq!(0, timer_state.cycles_done);
    }
    ...
    #[test]
    pub fn tick_to_zero() {
        let ppomo_config = PomoConfig::default();
        let mut timer_state = PomodoroTimer {
            duration_left_in_state: Duration::from_millis(1000),
            mode: Break,
            cycles_done: 0,
        ...
        assert_eq!(Break, timer_state.mode);
        assert_eq!(Duration::new(0, 0), timer_state.duration_left_in_state);
    }
    ...
    pub fn tick_and_move() {
        ...
        assert_eq!(Duration::from_millis(25 * 60 * 1000 - 1), timer_state.duration_left_in_state);
    }
}